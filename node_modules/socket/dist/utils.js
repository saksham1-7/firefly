...
function getHttpsAgent() {
  ...
  const agent = proxyUrl ? new vendor.HttpsProxyAgent({
    proxy: proxyUrl,
    ...(ca ? {
      ca,
      proxyRequestOptions: {
...
async function snapshotSocketFacts(scanPaths, options) {
  const {
    cwd,
    outputPath,
    tmpDir
  ...
  const sources = new Set(scanPaths.filter(p => path.basename(p) === DOT_SOCKET_DOT_FACTS_JSON).map(p => path.resolve(cwd, p)));
  sources.add(path.resolve(cwd, outputPath));
  const snapshotDir = path.join(tmpDir, 'socket-facts-snapshot');
  await fs$2.mkdir(snapshotDir, {
    recursive: true
  });
  const captured = [];
  const created = [];
  await Promise.all(Array.from(sources, async (source, index) => {
    ...
    try {
      await fs$2.copyFile(source, backup);
      captured.push({
        backup,
        source
      });
      ...
      debug$2.debugFn('warn', `[socket-facts] snapshot failed: ${source}`);
      debug$2.debugDir('error', e);
    }
  }));
  return {
    async restore() {
      await Promise.all([...captured.map(async ({
        backup,
        source
      }) => {
        try {
          await fs$2.copyFile(backup, source);
        } catch (e) {
          debug$2.debugFn('warn', `[socket-facts] restore failed: ${source}`);
          debug$2.debugDir('error', e);
        }
      }), ...created.map(source => fs$2.rm(source, {
        force: true
      }).catch(() => {}))]);
    }
  };
...
exports.snapshotSocketFacts = snapshotSocketFacts;
...
//# debugId=cf612c54-0957-4d6b-a5dd-fcf1ae189859
