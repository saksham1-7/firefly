...
async function outputCreateNewScan(result, options) {
  ...
  spinner?.stop();
  if (outputKind === 'json') {
    const payload = reachabilityFallback ? {
      ...result,
      reachabilityFallback
    } : result;
    logger.logger.log(utils.serializeResultJson(payload));
    if (wasSpinning) {
      spinner.start();
    }
    return;
  ...
  if (outputKind === 'markdown') {
    ...
    if (reachabilityFallback) {
      logger.logger.log(`Reachability analysis failed (${reachabilityFallback.message}); this Scan holds regular SCA results only.`);
      logger.logger.log('');
    }
    if (result.data.id) {
...
function attachPaths(entry, artifactPaths) {
  const coordKey = mavenCoordinateKey(entry.namespace, entry.name, entry.qualifiers?.['ext'], entry.qualifiers?.['classifier'], entry.version);
  if (!coordKey) {
    return {
      ...entry
    };
...
function accumulateSidecar(acc, facts, artifactPaths, factsFile) {
  acc.set(factsFile, {
    components: facts.components.map(comp => attachPaths(comp, artifactPaths)),
    projects: (facts.projects ?? []).map(proj => attachPaths(proj, artifactPaths))
  });
}
function hasResolvedPathsSidecarEntries(sidecar) {
  return Object.keys(sidecar).length > 0;
}
function hasSidecarEntries(acc) {
  return acc.size > 0;
}
...
function mergeResolvedPathsSidecars(a, b) {
  return {
    __proto__: null,
    ...a,
    ...b
...
async function performReachabilityAnalysis(options) {
  ...
  let sidecarPath;
  if (reachabilityOptions.dynamicSbomInference || resolvedPathsSidecar && hasResolvedPathsSidecarEntries(resolvedPathsSidecar)) {
...
async function findBuildToolCandidates({
  ...
  const resolvedHits = await Promise.all(hits.map(async hit => ({
    dir: await utils.realpathOrResolved(path.dirname(hit)),
  ...
  const realCwd = await utils.realpathOrResolved(cwd);
...
async function runManifestFacts({
  ...
  if (withFiles && sidecarAcc) {
    ...
    accumulateSidecar(sidecarAcc, facts, artifactPaths, await utils.realpathOrResolved(factsPath));
  }
...
async function runEcosystemCandidates({
  ...
  for (const [ecosystem, dirs] of candidatesByTool) {
    ...
    for (let dirIndex = 0; dirIndex < dirs.length; dirIndex += 1) {
      ...
      covered.add(dir);
      // eslint-disable-next-line no-await-in-loop
      const resolvedSubprojectDirs = await Promise.all(result.projects.map(project => utils.realpathOrResolved(path.resolve(dir, project.subprojectDir))));
...
async function generateRecursiveManifests({
  ...
  const runAll = resolvedSbtTmpDir => runEcosystemCandidates({
    candidatesByTool,
    excludePaths,
    realCwd,
    rootSockJson,
  ...
  const outcomes = sbtTmpDir ? await runAll(sbtTmpDir) : candidatesByTool.get('sbt')?.length ? await utils.withTmpDir('socket-sbt-facts-shared-', runAll) : await runAll(undefined);
  if (verbose) {
...
async function handleCreateNewScan({
  ...
  await utils.withTmpDir('socket-auto-manifest-', async manifestTmpDir => {
    ...
    if (reach.runReachabilityAnalysis) {
      ...
      const factsSnapshot = await utils.snapshotSocketFacts(packagePaths, {
        cwd,
        outputPath: constants.default.DOT_SOCKET_DOT_FACTS_JSON,
        tmpDir: manifestTmpDir
      });
      spinner.start();
      ...
      if (!reachResult.ok) {
        if (reach.reachFallbackToRegularScan) {
          ...
          logger.logger.warn(`Reachability analysis failed: ${reachResult.message}${reachResult.cause ? ` — ${reachResult.cause}` : ''}. Falling back to a regular SCA scan without reachability results.`);
        } else {
    ...
    try {
      fullScanCResult = await fetchCreateOrgFullScan(compressed.paths, orgSlug, {
        ...
        repoName,
        branchName,
        scanType: reach.runReachabilityAnalysis && !reachabilityFallback ? constants.default.SCAN_TYPE_SOCKET_TIER1 : constants.default.SCAN_TYPE_SOCKET,
        workspace
      }, {
        cwd,
        defaultBranch,
    ...
    } else if (reach.runReachabilityAnalysis && scanId && !tier1ReachabilityScanId && !reachabilityFallback) {
...
function detectCiPullRequestNumber() {
  const match = /^refs\/pull\/(\d+)\//.exec(env.envAsString(process.env['GITHUB_REF']));
...
async function handleManifestDynamicSbomInference({
  ...
  const result = !outcomes.length ? {
    ok: false,
    code: 1,
    message: 'No Gradle, sbt, or Maven build root was found beneath the given directory.',
    data: outcomes
...
async function markWorkspaceCoverage({
  ...
  const resolvedSubprojectDirs = await Promise.all(enumResult.projects.map(project => utils.realpathOrResolved(path.resolve(candidate.dir, project.subprojectDir))));
  for (const subprojectDir of resolvedSubprojectDirs) {
...
async function setupRecursiveManifestConfig(cwd, defaultOnReadError, excludePaths) {
  ...
  if (detectedEcosystems.length) {
    ...
    const hasAnyBuildRoot = ROOT_ECOSYSTEMS.some(ecosystem => (fullByTool.get(ecosystem)?.length ?? 0) > 0);
    if (!hasAnyBuildRoot) {
      return {
        ok: false,
        code: 1,
...
const reachabilityFlags = {
  ...
  reachFallbackToRegularScan: {
    type: 'boolean',
    default: false,
    description: 'If reachability analysis fails, continue with a regular SCA scan (without reachability results) instead of halting. By default, the CLI halts on reachability errors.'
  },
  reachLazyMode: {
    type: 'boolean',
    default: false,
...
const reachabilityFlagsForReach = {
  ...
  reachFallbackToRegularScan: {
    ...
    default: false,
    hidden: true,
    description: 'No-op: only `socket scan create --reach` falls back to a regular SCA scan on reachability failure.'
  }
};
const generalFlags = {
  ...flags.commonFlags,
...
//# debugId=7293854f-1543-486e-ba22-c269ec717113
